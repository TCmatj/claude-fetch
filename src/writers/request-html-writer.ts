...
function renderPromptBlocks(messages,query=''){
 const blocks=[];(Array.isArray(messages)?messages:[]).forEach(message=>{const content=Array.isArray(message?.content)?message.content:[message?.content];content.filter(Boolean).forEach(item=>blocks.push({item,role:message?.role}))});return blocks.map(({item,role})=>{const text=typeof item==='string'?item:item?.text??JSON.stringify(item,null,2);const isReminder=String(text).trimStart().startsWith('<system-reminder>');const roleKind=role==='user'?'user':'other';const cls=isReminder?'array-item prompt-reminder':roleKind==='user'?'array-item prompt-block':'array-item prompt-other';return renderBlock(cls,text,cacheControlOf(item),query,isReminder?undefined:role,roleKind)}).join('')
}
function renderBlock(cls,text,cacheControl,query,role,roleKind){return '<div class="'+cls+'">'+(role?'<div class="role-label role-'+esc(roleKind)+'">role: '+esc(role)+'</div>':'')+'<div class="array-body"><pre class="plain">'+hit(text,query.toLowerCase())+'</pre></div>'+renderCacheControl(cacheControl,query)+'</div>'}
